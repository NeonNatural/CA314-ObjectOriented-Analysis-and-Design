...
class Rack:
    ...
    def replace_tile(self,oldtile_index,newtile,new_pos):
        print("returining letter '{}' to rack".format(newtile))
        self.racktiles[oldtile_index] = Tiles(newtile,new_pos)

    def refill(self):
        if 7 - self.rack_len() > 0:
            #print("refilling...")
            for i in (self.bag.take(7 - self.rack_len())):
                print(i.get_letter())
                self.racktiles.append(i)
